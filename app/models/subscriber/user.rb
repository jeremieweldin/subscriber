module Subscriber
  class User < ActiveRecord::Base
    ...
    validates :password, :format => {:with => /^.*(?=.{10,})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=]).*$/, message: "must be at least 6 characters and include one capital and lowercase letter, one number, and one special character."}
